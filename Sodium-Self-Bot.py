...
@darks.command()
async def addy(ctx):
    message = (f"<:stolen_emoji:1279223176475447379> **LTC ADDY** <:stolen_emoji:1279223176475447379> ")
    message2 = (f"{LTC}")
...
# ABUSE
@darks.command()
async def lodepe(ctx):
    message = '**HIJARDU TERI MUMMY KI CHUT MAI LODA MARU RAAAAND HR , NR , MR , NT , INY , LODE PE DARKS PAPA ON TOP!! , 360 DEGREE PE RAKH KE MAA CHODU JHAANT BARABAR KE PILLO KI YAAD KAREINGY MAA YOWAYYY AUR UPAR SE LINDUSTANI BHI DARKS KE LODE PE , NUKERS RUNS CORD NCTOP!!!! , BATEIN BUTEIN BHULJA DARKS KA PAKARH AUR JHOOL JA!! WROTE BY DSC.GG/dr-op**'
    await ctx.send(message)
    await ctx.message.delete()        
...
async def selfbot(ctx):
    ...
    server_link = "https://discord.gg/fMXa2TD7"
...
@darks.command()
async def machoddi(ctx, user: discord.Member):
...
async def level(ctx):
    await ctx.message.delete()
    responses = [
        'Cry about it', 'Darks The Best Coder Yumm Yumm', 'We love you Darks',
        'Darks Ke Haters Maa Chudao', 'Baki CoderZ The Lumber One Skidders Darks OP 🥶', 'Shut the up LUND LELE RANDIIII DARKS OP BAKI SAB TOPI'
    ]
